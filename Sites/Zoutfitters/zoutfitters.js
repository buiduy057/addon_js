...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.4zoutfitters.com") > -1 ||
  window.location.href.indexOf("https://4zoutfitters.com") > -1
) {
  $(window).ready(function () {
    var url_string = window.location.href;
    async function getPage(page = 105) {
      try {
        let urlProducts = [];
        console.log(`page=${page}`);
        let linkapi1 =
          "https://www.4zoutfitters.com/api/catalog/products_v2.json?sort_field=id&sort_direction=desc&limit=12&page=" +
          page +
          "&minimal=true";
        ...
        for (const path of urlsUnique) {
          try {
            // console.log(path);
            const urlProduct = `https://www.4zoutfitters.com/${path}`;
