...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.dinkstar.com") > -1 ||
  window.location.href.indexOf("https://dinkstar.com") > -1
) {
  $(window).ready(function () {
    if (window.location.href.indexOf("/shop/") > -1) {
      ...
      var url_string = window.location.href;
      let page = 0;
      async function getPage(page = 121) {
        try {
          ...
          for (const path of urlsUnique) {
            try {
              ...
              let title = $(bodyPath)
                ...
                .text();
              // console.log(title);
              let description = $(bodyPath).find(".tabbed-content").html();
              // Object.keys(myObj).length;
              let var_img = [];
              let img = $(bodyPath).find(
                ".shop-container .product-thumbnails img"
              );
              $(img).each(function (_, item) {
                var_img.push($(item).attr("src"));
              });
              const dataProduct = $(bodyPath)
                .find("form.cart")
                .attr("data-product_variations");
              const data_variant = JSON.parse(dataProduct);
              // console.log(data_variant);
              let option1_name = "";
              let option2_name = "";
              let variants = [];
              if (Object.keys(data_variant[0].attributes).length > 1) {
                option1_name = "Styles";
                option2_name = "Dimension";
                for (const item of data_variant) {
                  const itemVariant = {
                    option1: item.attributes.attribute_pa_types,
                ...
                if (
                  data_variant[0].attributes.attribute_pa_types !== undefined
                ) {
                  option1_name = "Styles";
                  for (const item of data_variant) {
                    const itemVariant = {
              ...
              product["gender"] = "All";
              product["vender"] = "dinkstar";
              product["vender_url"] = path;
              product["description"] = description;
              product["images"] =
                var_img.length > 1 ? var_img : variants[0].var_images;
