...
// https://blackgirlsshop
else if (
  window.location.href.indexOf("blackgirlsshop.com") > -1 ||
  window.location.href.indexOf("https://www.blackgirlsshop.com") > -1
) {
  console.log("blackgirlsshop");
  if (window.location.href.indexOf("https://blackgirlsshop.com/#MainContent") > -1) {
    window.close();
    ...
    $(window).on("load", function () {
      (async () => {
        ...
        listImagevar = null;
        const trvar = $("table.variations tbody tr");

        $(trvar).each(function (index, value) {
          const name = $(value).find("td").eq(0).find("label").text();
          optionNames[`option${index + 1}_name`] = name;
        });

        const variants = [];
        const { option1_name, option2_name, option3_name, option4_name } = optionNames;

        if (option1_name) {
          const op1Variants = $(trvar).find("td.value").eq(0).find("select option:not(:first)");
          const op2Variants = $(trvar).find("td.value").eq(1).find("select option:not(:first)");
          const op3Variants = $(trvar).find("td.value").eq(2).find("select option:not(:first)");
          const op4Variants = $(trvar).find("td.value").eq(3).find("select option:not(:first)");

          $(op1Variants).each(function (_1, vt1) {
            let vtsItems = {
              option1: $(vt1).text(),
              origin_price: true,
              price: 1,
            ...
            if (option2_name) {
              $(op2Variants).each(function (_2, vt2) {
                vtsItems = { ...vtsItems, option2: $(vt2).text() };
                if (option3_name) {
                  $(op3Variants).each(function (_3, vt3) {
                    vtsItems = { ...vtsItems, option3: $(vt3).text() };

                    if (option4_name) {
                      $(op4Variants).each(function (_4, vt4) {
                        vtsItems = { ...vtsItems, option4: $(vt4).text() };
                        variants.push(vtsItems);
                      });
                    } else {
                      variants.push(vtsItems);
                    }
