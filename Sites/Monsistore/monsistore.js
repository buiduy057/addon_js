...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.monsistore.com") > -1 ||
  window.location.href.indexOf("https://monsistore") > -1
) {
  if (window.location.href.indexOf("/shop") > -1) {
    $(window).ready(function () {
      var url_string = window.location.href;
      async function getPage(page = 106) {
        try {
          let urlProducts = [];
            ...
            try {
              ...
              if (dataJson !== undefined) {
                ...
                if (Object.keys(dataVariant[0].attributes).length > 1) {
                  if (
                    dataVariant[0].attributes.attribute_product !== undefined
                  ) {
                    // const dataVar = JSON.parse(dataVariant);
                    // console.log(dataVar);
                    option1_name = "Styles";
                    option2_name = "Dimension";
                    for (const item of dataVariant) {
                      const itemVariant = {
                        option1: item.attributes.attribute_product,
                    ...
                    product["gender"] = "All";
                    product["vender"] = "monsistore";
                    product["vender_url"] = path;
                    product["description"] = description;
                    product["images"] =
                      imageThub.length > 1 ? imageThub : images;
                    ...
                    send(product, "monsistore");
                  } else {
                    let datastringify = JSON.stringify(dataVariant);
                    // console.log(datastringify);
                    let dataProduct_01 = datastringify.replace(
                      /attribute_pa_size-shoes/gi,
                      "attribute_pa_size_shoes"
                    );
                    let dataProduct_02 = dataProduct_01.replace(
                      /attribute_pa_size-type/gi,
                      "attribute_pa_size_type"
                    );
                    const dataVar = JSON.parse(dataProduct_02);
                    // console.log(dataVar);
                    option1_name = "Styles";
                    option2_name = "Dimension";
                    for (const item of dataVar) {
                      const itemVariant = {
                        option1: item.attributes.attribute_pa_size_type,
                        option2: item.attributes.attribute_pa_size_shoes,
                        price: item.display_price - 2,
                        origin_price: true,
                        shipping_cost: 0,
                        quantity: 9999,
                  ...
                  product["vender_url"] = path;
                  product["description"] = description;
                  product["images"] = imageThub.length > 1 ? imageThub : images;
                  product["variants"] = variants_arr;
                  product["designed"] = -1;
                  product["trending"] = trending === true ? 1 : 0;
                  product["feedback"] = null;
                ...
                let priceSale = $(bodyPath)
                  .find(" form.cart .price-on-sale ins bdi")
                  .text();
                let price = priceSale.replace("$", "");
                const itemVariant = {
                  price: price,
                  origin_price: true,
                  shipping_cost: 0,
