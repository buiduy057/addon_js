...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.melaningift.com") > -1 ||
  window.location.href.indexOf("https://melaningift.com") > -1
) {
    ...
    if (window.location.href.indexOf("/tags/") > -1) {
      ...
      async function getPage(page = 1) {
        try {
          ...
          const linkapi1 = `https://www.melaningift.com/rest/retail-products/groups/5f5f6de99e094d7f1d8835db/tag/${category}?limit=36&page=${page}recentViewAsShould=true`;
          ...
          for (const item of data) {
            ...
            const url_href = `https://www.melaningift.com/campaigns/-/-/tags/${category}/${product_href}?retailProductCode=${code_href}`;
            ...
            try {
              ...
              for (let i = 0; i <= data.length - 1; i++) {
                if (data[i].campaignUrl === category_path) {
                  ...
                  let vender_url = `https://www.melaningift.com/campaigns/-/-/tags/${category}/${data_item.campaignUrl}?retailProductCode=${data_item.code}`;

                  const linkapi3 = `https://www.melaningift.com/rest/retail-products/by-campaign/${data[i].campaignUrl}?groupId=5f5f6de99e094d7f1d8835db`;

                  const request_price = await fetch(linkapi3);
                  const variants_price = await request_price.json();
                  // console.log(variants_price);

                  const linkapi2 = `https://www.melaningift.com/rest/products/by-campaign/${data[i].campaignUrl}?groupId=5f5f6de99e094d7f1d8835db`;
  ...
  for (let i = 0; i <= arr.length - 1; i++) {
    if ((name === arr[i].names.product) & (color === arr[i].color)) {
      ...
      if (front.indexOf("front") !== -1) {
        let rp_back = item.replace("front", "back");
