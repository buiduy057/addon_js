...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.kulhk.com") > -1 ||
  window.location.href.indexOf("https://kulhk.com") > -1
) {
  $(window).ready(function () {
    ...
    async function getPage(page = 1) {
      try {
        ...
        if (page_arr[0] !== null) {
          const urlRequest = `https://kulhk.com/api/product/v2/collections/60065e7c16e77c98172b9562/products?startId=${page_arr[0]}&limit=12`;
          ...
          for (const path of body.data.products) {
            const item = `https://kulhk.com/${path.slug}?collection=apparel`;
          ...
          for (const path of urlsUnique) {
            try {
              ...
              let images_arr = [];
              for (const item of dataJson.gallery) {
              ...
              const rqVar = await fetch(
                `https://kulhk.com/api/product/products/${id}/options`
              );
