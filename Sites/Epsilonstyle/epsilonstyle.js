...
if (window.location.href.indexOf("https://xx.com") > -1) {
  ...
  window.location.href.indexOf("https://www.epsilonstyle.com") > -1 ||
  window.location.href.indexOf("https://epsilonstyle.com") > -1
) {
    ...
    if (window.location.href.indexOf("/tags/") > -1) {
      ...
      async function getPage(page = 1) {
        try {
          ...
          const linkapi1 = `https://www.epsilonstyle.com/rest/retail-products/groups/5e0037a1a9d73c2dda0201a3/tag/${category}?limit=36&page=${page}recentViewAsShould=true`;
          ...
          for (const item of data) {
            ...
            const url_href = `https://www.epsilonstyle.com/campaigns/-/-/tags/${category}/${product_href}?retailProductCode=${code_href}`;
              ...
              for (let i = 0; i <= data.length - 1; i++) {
                if (data[i].campaignUrl === category_path) {
                  ...
                  if (var_url.indexOf(title) === -1) {
                    ...
                    let vender_url = `https://www.epsilonstyle.com/campaigns/-/-/tags/${category}/${data_item.campaignUrl}?retailProductCode=${data_item.code}`;
                    const linkapi3 = `https://www.epsilonstyle.com/rest/retail-products/by-campaign/${data[i].campaignUrl}?groupId=5e0037a1a9d73c2dda0201a3`;
                    const request_price = await fetch(linkapi3);
                    const variants_price = await request_price.json();
                    // console.log(variants_price);
                    const linkapi2 = `https://www.epsilonstyle.com/rest/products/by-campaign/${data[i].campaignUrl}?groupId=5e0037a1a9d73c2dda0201a3`;
                    ...
                    if (variants_arr[0].var_images[1] !== undefined) {
